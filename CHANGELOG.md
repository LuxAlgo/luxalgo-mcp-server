...
- `LUXALGO_REACTIVE_AUTH_ONLY` and `LUXALGO_SECURITY_SCHEMES`, both added while chasing #1013 and never released. Hiding the well-known PRM for the whole origin fixed Claude but broke every standard client (ChatGPT's app submission: "couldn't detect OAuth metadata"); the Claude-only host above does the same thing without touching the standard one. `securitySchemes` was ruled out as the trigger and is now always on for the standard host.
