...
```
request to /.well-known/oauth-protected-resource[/mcp]    (standard host; the Claude host: /auth/prm)
...
**A sign-in popup at connect time in Claude.ai is not a bug in this server.** Verified in production logs (Sep 2026): Claude runs `initialize` and `tools/list` anonymously (both 200, no challenge from us), then fetches `/.well-known/oauth-protected-resource/mcp` on its own and starts OAuth — with `securitySchemes` already off, so the probe is its only input. Anthropic's own [lazy-authentication guide](https://claude.com/docs/connectors/building/lazy-authentication) describes this server's exact shape and says the connect-time prompt should not occur with *Required when the server asks*; the discrepancy is tracked as [anthropics/claude-ai-mcp#1013](https://github.com/anthropics/claude-ai-mcp/issues/1013).

Until #1013 is fixed the workaround is a **second host for Claude only**, `https://claude.mcp.luxalgo.com/mcp` (`CLAUDE_MCP_RESOURCE` in `src/auth/config.ts`; the host is picked per request in `src/auth/surface.ts`). It serves the same tools, but its discovery is purely reactive — the trick from the official `modelcontextprotocol/ext-apps` lazy-auth example's `REACTIVE_AUTH_ONLY` flag: the well-known PRM paths answer 404, the document lives at `/auth/prm` (a URL clients only learn from the 401's `resource_metadata`), and `tools/list` carries no `securitySchemes`. Every other path — `/register` included — answers 404, exactly like a genuinely authless origin: the example instead answers `POST /register` with an RFC 7591 "not supported" error, but claude.ai surfaces that 400 as a fatal "couldn't register with the sign-in service" (observed Sep 2026) rather than connecting without auth. The Claude host is its own resource identifier, so tokens minted for it carry its audience and are refused on the standard host (and vice versa); the app lists both as resources. `https://mcp.luxalgo.com/mcp` stays standard RFC 9728 — never hide its metadata again: an origin-wide version of this trick shipped briefly and broke ChatGPT's app submission ("couldn't detect OAuth metadata"). Do not loosen the gate itself, and do not weaken lazy auth in any other way. Remove the Claude host (config, `surface.ts`, the app's resource, the DNS record) once #1013 is fixed.
