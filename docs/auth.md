...
Until #1013 is fixed there is one sanctioned counter-measure, `LUXALGO_REACTIVE_AUTH_ONLY=on` (see `src/auth/config.ts`), ported from the official `modelcontextprotocol/ext-apps` lazy-auth example's `REACTIVE_AUTH_ONLY` flag: the well-known PRM paths answer 404 and the document moves to `/auth/prm`, a URL clients only learn from the 401's `resource_metadata`. Every other path — `/register` included — answers 404, exactly like a genuinely authless origin: the example instead answers `POST /register` with an RFC 7591 "not supported" error, but claude.ai surfaces that 400 as a fatal "couldn't register with the sign-in service" (observed Sep 2026) rather than connecting without auth. This deliberately violates RFC 9728 §3 — a client that probes the well-known path instead of honouring `resource_metadata` cannot sign in while it is on — so after flipping it, verify sign-in still works in Claude Code, Cursor and ChatGPT. Default stays off (standard RFC 9728). Do not loosen the gate itself, and do not weaken lazy auth in any other way — point people at the README's Claude install note instead.
